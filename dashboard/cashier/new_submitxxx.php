...
if (isset($_POST['p_name']) && isset($_POST['mem_type']) && isset($_POST['total']) && isset($_POST['age']) && isset($_POST['paid'])) {
    ...
    $birthdate   = rtrim($_POST['birthdate']);
    ...
    if (mysqli_affected_rows($con) == 1) {
        while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
            ...
            mysqli_query($con, "UPDATE user_data SET name='$full_name', address='$address', contact='$contact', email='$email', height='$height', weight='$weight', zipcode='$zipcode', birthdate='$birthdate', nationality='$nationality', facebookaccount='$facebookaccount', twitteraccount='$twitteraccount', contactperson='$contactperson', previousgym='$previousgym', yearstraining='$yearstraining', joining='$date', age='$age', proof='$proof', other_proof='$other_proof', sex='$sex' WHERE wait='yes'");
            $bal = $total - $paid;
...
?>";
	...
	<body>
		<header>
			...
			<address>
        <p>Body Fuel Gym</p>
        <p>Amrapali Axiom below Dominoz Pizza</p>
        <p>Landline:+02717234042</p><p>www.ankittaral211094@gym.com</p><br><p><div id="barcodeTarget" class="barcodeTarget"></div>
    <canvas id="canvasTarget"></canvas> </span>
      </address>
			<span><img alt="" src="../../img/icon.png">
		</header>
		<article>
			<table class="meta">
...
?></span></td>
		...
		<aside>
			<h1><span  >Additional Notes</span></h1>
			 <div  >
        <p>1). All members must abide our TnC / rules otherwise membership may be withdrawn. </br></br>2). The payment is not transferable and non-refundable.</br></br>3).Fee Should be submitted within 5 business days before subjected to expiration, otherwise membership would be cancelled.</br></br>4).Members are compulsory to wear shoes.</br></br>5).Smoking will not permitted within gym periheral.</br></br>6). If you do harm any gym property prepare for the consequences.
      </div>
    </aside><center><br><br><a href="view_mem.php">Body Fuel Gym (ankittaral211094@gmail.com)</center>
	</body>
</html>